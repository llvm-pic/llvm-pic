...
class PICMidObjectTargetWriter : public MCELFObjectTargetWriter {
  int Val;

public:
  PICMidObjectTargetWriter()
      : MCELFObjectTargetWriter(false, 0, ELF::EM_MCHP_PIC, false),
        Val(ELF::EM_MCHP_PIC) {}

  ~PICMidObjectTargetWriter() override { Val = 0; }
...
PICMidMCAsmBackend::createObjectTargetWriter() const {
  return std::make_unique<PICMidObjectTargetWriter>();
}
